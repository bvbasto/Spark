...
csvFile = "/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/WDIData.csv"
...
destDataDir = "/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/parquet/data_csv"
df3.repartition(1).write.mode('overwrite').option("header", "true").csv(destDataDir)

# COMMAND ----------

destDataDir = "/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/parquet/data_part"
df3.repartition('idIndicator').write.mode('overwrite').partitionBy('idIndicator').parquet(destDataDir)

# COMMAND ----------

destDataDir = "/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/parquet/bvb"
...
# MAGIC LOCATION '/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/parquet/bvb';
...
cls = ('Indicator Code','Indicator Name')
destDataDir = "/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/parquet/indicators"
...
# MAGIC USING parquet
# MAGIC --partitioned by (y,m)
# MAGIC LOCATION '/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/parquet/indicators';
# MAGIC 
# MAGIC SELECT * FROM wdb_indicators limit 10;

# COMMAND ----------

csvFileC = "/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/WDICountry.csv"
destDataDir = "/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/parquet/countries"
...
# MAGIC  )
# MAGIC USING parquet
# MAGIC LOCATION '/mnt/MyADLS/training/_bvb/WorldDataBank/WorldDevelopmentIndicators/parquet/countries';
# MAGIC 
...
# MAGIC %sql
# MAGIC 
# MAGIC SELECT 5 as n, Ano,sum(Valor) FROM wdb_data where idIndicator = 'SI.DST.05TH.20' and idCountry = 'WLD'  GROUP BY ano 
# MAGIC UNION
# MAGIC SELECT 1,Ano, sum(Valor) FROM wdb_data where idIndicator = 'SI.DST.FRST.20' and idCountry = 'WLD'  GROUP BY ano 
# MAGIC ORDER BY ano,n
