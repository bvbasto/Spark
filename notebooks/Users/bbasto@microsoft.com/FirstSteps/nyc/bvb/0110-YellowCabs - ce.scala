...
val srcDataDirRoot = "/mnt/MyADLS/nyc-taxi/transactional-data/" 
val destDataDirRoot = "/mnt/MyADLS/nyctaxi/rawDir/yellow-taxi" 
...
// MAGIC LOCATION '/mnt/MyADLS/nyctaxi/rawDir/yellow-taxi/';
...
val f = srcDataDirRoot + "year=2017/month=09/type=yellow/yellow_tripdata_2017-09.csv"      

val taxiDF = sqlContext.read.format("csv")
                      .option("header", "true")
                      .schema(yellowTripSchema2017H1)
                      .option("delimiter",",")
                      .load(f).cache()
...
for (j <- 2018 to 2018)
  {
    val endMonth = if (j==2018) 6 else 12 
    for (i <- 1 to endMonth) 
    {
      //Source path  
...
// MAGIC %sql
// MAGIC SELECT  trip_year, trip_month ,count(*) from yellow_taxi_trips GROUP BY trip_year, trip_month order by trip_year desc, trip_month desc
